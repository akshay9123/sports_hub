...
// ItemMasterSchema.pre("validate", function (next) {
//   if (this.warranty === true) {
//     const hasStandardWarranty =
//       this.firstyearwarranty || this.secyearwarranty || this.thirdyearwarranty;

//     const hasCustomWarranty =
//       Array.isArray(this.customWarranty) && this.customWarranty.length > 0;

//     if (!hasStandardWarranty && !hasCustomWarranty) {
//       return next(
//         new Error(
//           "If warranty is enabled, you must provide at least one warranty option (year-based or custom warranty)."
//         )
//       );
//     }
//   }
...
const ItemMasterSchema = new mongoose.Schema(
  {
    ...
    firstyearwarranty: { type: String },
    secyearwarranty: { type: String },
    thirdyearwarranty: { type: String },
    ...
    // PURCHASE CONFIG
    purch_desc: { type: String },
    purchase_rate: { type: Number },

    // STOCK CONTROL (NEW – IMPORTANT)
    minimum_level: { type: Number, default: 0 },
    maximum_level: { type: Number, default: 0 },

    // System stock is now derived from StoreStock model
    track_inventory: {
      type: Boolean,
      default: true,
    },

    batch_wise_inventory: { type: Boolean, default: false },
    batch_wise_rate: { type: Boolean, default: false },

    // RACK / LOCATION
    rackbin_no: { type: String },
...
ItemMasterSchema.pre("validate", function (next) {
  if (this.warranty === true) {
    ...
    if (!hasStandardWarranty && !hasCustomWarranty) {
      return next(
        new Error(
          "If warranty is enabled, provide at least one warranty option."
        )
      );
    }
  }
...
ItemMasterSchema.pre("save", async function (next) {
  ...
  try {
    ...
    if (lastItem && lastItem.code) {
      const nextCodeNum = Number(lastItem.code) + 1;
      newCode = nextCodeNum.toString().padStart(5, "0");
    }
